...
#define num_turning_radii   (1)
                ...
                int numero, double t, double u, double v, trajectory_t& traj)
        {
            ...
            {
                double s[3] = {(double)px[i], (double)py[i], (double)pth[i]};
                ...
                reeds_shepp_optimization_data_t& opt_data)
        {
            ...
            for(int i=num_turning_radii-1; i >=0; i--)
            {
                ...
                int numero;
                double t, u, v;
                double len = min_length_rs(si[0], si[1], si[2],
                        sf[0], sf[1], sf[2],
                        &numero, &t, &u, &v);

                double cost = len + rho*1./tr; 
                if(cost < min_cost)
                {
                    min_cost = cost;
        ...
        void test_extend_to()
        {
            ...
            double zero[3] ={0};
            state_t origin(zero);
            double goal[3] = {0,0, 175/180.0*M_PI};
            state_t sr(goal);
